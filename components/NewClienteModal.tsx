...
const NewClienteModal: React.FC<Props> = ({ onClose, onSuccess }) => {
    const [formData, setFormData] = useState<Partial<NewClienteData>>({});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.nombre || !formData.empresa || !formData.nit) {
            setError('Nombre, empresa y NIT son obligatorios.');
            return;
        }

        setLoading(true);
        try {
            await createCliente(formData as NewClienteData);
            onSuccess();
        } catch (err: any) {
         ...
         <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <input name="nombre" placeholder="Nombre del Cliente *" onChange={handleChange} className="w-full p-2 border rounded" required />
                    <input name="empresa" placeholder="Empresa *" onChange={handleChange} className="w-full p-2 border rounded" required />
                    <input name="nit" placeholder="NIT *" onChange={handleChange} className="w-full p-2 border rounded" required />
